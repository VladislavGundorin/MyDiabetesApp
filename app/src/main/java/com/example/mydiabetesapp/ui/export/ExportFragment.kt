...
class ExportFragment : Fragment() {
    ...
    ) = FragmentExportBinding.inflate(inflater, container, false).also { _binding = it }.root

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val db   = AppDatabase.getDatabase(requireContext())
        val repo = ReportRepository(db.reportDao(), db.glucoseDao(), db.weightDao())
        ...
        binding.btnGenerate.setOnClickListener { vm.generate(requireContext(), start, end) }

        binding.rvReports.layoutManager = LinearLayoutManager(requireContext())
        binding.rvReports.adapter = adapter

        viewLifecycleOwner.lifecycleScope.launch {
            vm.reports.collect { adapter.submitList(it) }
        }
    ...
    private fun openFile(report: ReportEntry) {
        val file = File(Uri.parse(report.uri).path!!)
